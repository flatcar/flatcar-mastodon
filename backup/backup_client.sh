...
mastodon_paths=("/opt/mastodon/public"
                "/opt/mastodon/elasticsearch"
                "/opt/mastodon/redis"
                "/opt/mastodon/mastodon.env"
                "/opt/mastodon/versions.env")

monitoring_paths=("/opt/mastodon/grafana"
                  "/opt/mastodon/prometheus"
                  "/opt/mastodon/monitoring-versions.env"
                  "/opt/mastodon/monitoring-admin.env"
                  "/opt/mastodon/postgres-monitoring-queries.yaml")

extra_paths=("/opt/backup"
             "/opt/mastodon/wait-for-container.sh"
             "/opt/mastodon/setup-mastodon.sh")

service_wildcards=("/etc/systemd/system/mastodon*"
